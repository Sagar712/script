...
function exportFile() {
    ...
    let scriptDb = JSON.parse(localStorage.getItem("ScriptAppData"));
    let listDb = JSON.parse(localStorage.getItem("ListTableData"));
    if(scriptDb != null)
        masterDb.tasks = scriptDb.tasks;
    if(listDb != null)
        masterDb.items = listDb.items;
    console.log(masterDb);
    download(JSON.stringify(masterDb), "Back-Up-Data.json", "text/plain");
}
